 ...
 * 
 * <h2>Notes:</h2>
 * I don't think there's any significant way to speed this one up. It's possible to reuse the left and
 * right figures, similar to what we did in {@link ProjectEuler.Problem008}, but the gain is not significant.
 * 
 * <p>
 *
...
public class Problem011 {
    ...
    public static void main(String[] args) {
        int greatest = 0;
        int product = 0;
        int[][] nums = {{8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8},
        ...
        for (int i = 3; i < nums.length - 3; i++)
            for (int j = 3; j < nums[i].length - 3; j++) {
                // North
                if ( (product = nums[i][j] * nums[i - 1][j] * nums[i - 2][j] * nums[i - 3][j]) > greatest)
                    greatest = product;
                // NorthEast
                if ( (product = nums[i][j] * nums[i - 1][j + 1] * nums[i - 2][j + 2] * nums[i - 3][j + 3]) > greatest)
                    greatest = product;
                // East
                if ( (product = nums[i][j] * nums[i][j + 1] * nums[i][j + 2] * nums[i][j + 3]) > greatest)
                    greatest = product;
                // SouthEast
                if ( (product = nums[i][j] * nums[i + 1][j + 1] * nums[i + 2][j + 2] * nums[i + 3][j + 3]) > greatest)
                    greatest = product;
